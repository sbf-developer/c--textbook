...
\begin{itemize}
  \item Explain the central problem addressed by Debuggers and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Debuggers; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
A debugger observes a running process through breakpoints, stack frames, variables, and watchpoints. Observation changes timing, so a debugger is evidence, not an infallible explanation of concurrent behavior.
...
The complete standalone program for this chapter is stored at examples/chapters/ch101.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch101.cpp}
