\chapter{Interoperability with C}
\index{Interoperability with C}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by Interoperability with C and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Interoperability with C; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
C interoperability requires a compatible data and calling boundary, usually with extern "C" and C-compatible types. Ownership and error conventions must cross the boundary explicitly.
...
The complete standalone program for this chapter is stored at examples/chapters/ch141.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch141.cpp}
