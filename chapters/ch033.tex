\chapter{std::string and std::string\_view}
\index{std::string and std::string\_view}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by std::string and std::string\_view and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: std::string and std::string\_view; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
std::string owns text storage; std::string\_view borrows a range of characters. A view is efficient only when its source outlives every use of the view.
...
The complete standalone program for this chapter is stored at examples/chapters/ch033.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch033.cpp}
