...
\begin{itemize}
  \item Explain the central problem addressed by const Correctness and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: const Correctness; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
const communicates non-mutation through a particular access path. It is a useful local guarantee, but it does not make pointed-to objects immutable or make concurrent access safe.
...
The complete standalone program for this chapter is stored at examples/chapters/ch053.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch053.cpp}
