...
\begin{itemize}
  \item Explain the central problem addressed by std::array and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: std::array; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
std::array gives a fixed-size sequence with value semantics, standard-library interoperability, and a size that is part of the type. It is often a better default than a built-in array.
...
The complete standalone program for this chapter is stored at examples/chapters/ch031.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch031.cpp}
