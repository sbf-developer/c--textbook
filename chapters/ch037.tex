...
\begin{itemize}
  \item Explain the central problem addressed by Dynamic Storage and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Dynamic Storage; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Dynamic storage supports objects whose size or lifetime is not conveniently tied to a local scope. Modern C++ hides raw allocation behind containers and resource-owning types whenever possible.
...
The complete standalone program for this chapter is stored at examples/chapters/ch037.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch037.cpp}
