\chapter{Selection with if and switch}
\index{Selection with if and switch}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by Selection with if and switch and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Selection with if and switch; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Selection chooses behavior based on a condition or a discrete value. Good branches make invalid states visible and keep policy separate from mechanism.
...
The complete standalone program for this chapter is stored at examples/chapters/ch017.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch017.cpp}
