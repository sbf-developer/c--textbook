...
\begin{itemize}
  \item Explain the central problem addressed by Static Analysis and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Static Analysis; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Static analyzers inspect code without executing every path. Warnings are hypotheses that require triage, not proof that every reported line is wrong.
...
The complete standalone program for this chapter is stored at examples/chapters/ch103.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch103.cpp}
