\chapter{Customization and Generic Interfaces}
\index{Customization and Generic Interfaces}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by Customization and Generic Interfaces and relate it to the C++ object, type, or program model.
  \item Use the idea in a small program, identify its main failure modes, and state one trade-off honestly.
\end{itemize}
\textbf{Prerequisites}: the preceding chapter, plus the vocabulary introduced in Part 12.

\textbf{Key terminology}: Customization and Generic Interfaces; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Customization points let generic code adapt to user types without hard-coding every type. A good customization protocol documents discovery, precedence, and semantic requirements.
...
The complete standalone program for this chapter is stored at examples/chapters/ch170.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch170.cpp}
