...
\begin{itemize}
  \item Explain the central problem addressed by Lambda Expressions and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Lambda Expressions; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Lambdas create local callable objects with explicit capture and parameter rules. Capture is a lifetime decision, especially when a callable escapes its defining scope.
...
The complete standalone program for this chapter is stored at examples/chapters/ch070.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch070.cpp}
