\chapter{Undefined, Unspecified, and Implementation-Defined Behavior}
\index{Undefined, Unspecified, and Implementation-Defined Behavior}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by Undefined, Unspecified, and Implementation-Defined Behavior and relate it to the C++ object, type, or program model.
  \item Use the idea in a small program, identify its main failure modes, and state one trade-off honestly.
\end{itemize}
\textbf{Prerequisites}: the preceding chapter, plus the vocabulary introduced in Part 3.

\textbf{Key terminology}: Undefined, Unspecified, and Implementation-Defined Behavior; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
These labels describe different degrees of freedom in the standard. Correct engineering avoids undefined behavior, documents implementation choices, and does not mistake one observed result for a guarantee.
...
The complete standalone program for this chapter is stored at examples/chapters/ch045.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch045.cpp}
