...
\begin{itemize}
  \item Explain the central problem addressed by Error Codes and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Error Codes; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Error codes make failure part of a value-level protocol. They are explicit and useful at low-level or performance-sensitive boundaries, but callers must not ignore them.
...
The complete standalone program for this chapter is stored at examples/chapters/ch094.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch094.cpp}
