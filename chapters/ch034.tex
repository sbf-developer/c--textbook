...
\begin{itemize}
  \item Explain the central problem addressed by References and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: References; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
A reference is an alias with binding rules, not a reseatable pointer and not an ownership token. References make non-null borrowing expressive but do not extend lifetime automatically.
...
The complete standalone program for this chapter is stored at examples/chapters/ch034.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch034.cpp}
