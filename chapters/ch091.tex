\chapter{Syntax, Linker, Logic, and Runtime Errors}
\index{Syntax, Linker, Logic, and Runtime Errors}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by Syntax, Linker, Logic, and Runtime Errors and relate it to the C++ object, type, or program model.
  \item Use the idea in a small program, identify its main failure modes, and state one trade-off honestly.
\end{itemize}
\textbf{Prerequisites}: the preceding chapter, plus the vocabulary introduced in Part 7.

\textbf{Key terminology}: Syntax, Linker, Logic, and Runtime Errors; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Different failure classes arise at different stages. Classification narrows the search and prevents treating every failure as a compiler problem.
...
The complete standalone program for this chapter is stored at examples/chapters/ch091.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch091.cpp}
