...
\index{Condition Variables}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by Condition Variables and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Condition Variables; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
A condition variable lets a thread sleep until shared state may have changed. Waiting must always re-check a predicate because wakeups are not proofs that the condition holds.
...
The complete standalone program for this chapter is stored at examples/chapters/ch147.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch147.cpp}
