...
\begin{itemize}
  \item Explain the central problem addressed by std::vector and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: std::vector; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
std::vector owns a contiguous dynamic sequence. Size, capacity, iterator invalidation, and reallocation determine which references remain valid after a mutation.
...
The complete standalone program for this chapter is stored at examples/chapters/ch032.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch032.cpp}
