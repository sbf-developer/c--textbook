\chapter{Source Code, Machine Code, and Executable Files}
\index{Source Code, Machine Code, and Executable Files}
\begin{chaptermeta}
\textbf{Learning objectives}
\begin{itemize}
  \item Explain the central problem addressed by Source Code, Machine Code, and Executable Files and relate it to the C++ object, type, or program model.
  \item Use the idea in a small program, identify its main failure modes, and state one trade-off honestly.
\end{itemize}
\textbf{Prerequisites}: the preceding chapter, plus the vocabulary introduced in Part 1.

\textbf{Key terminology}: Source Code, Machine Code, and Executable Files; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
Human-readable source is translated into target-specific object code and eventually an executable image. The chapter explains why the same source can produce different binaries without making portability mysterious.
...
The complete standalone program for this chapter is stored at examples/chapters/ch003.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch003.cpp}
