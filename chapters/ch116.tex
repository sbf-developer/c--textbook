...
\begin{itemize}
  \item Explain the central problem addressed by Heaps and relate it to the C++ object, type, or program model.
...
\textbf{Key terminology}: Heaps; contract; representation; lifetime; invariant; diagnostic; trade-off.
\end{chaptermeta}

\section{The problem and the mental model}
A heap maintains a partial order that makes an extreme element accessible. It is useful for priority queues and selection, not for producing a fully sorted sequence by itself.
...
The complete standalone program for this chapter is stored at examples/chapters/ch116.cpp and is compiled by the verification script.

\inputminted[fontsize=\scriptsize,breaklines]{cpp}{examples/chapters/ch116.cpp}
